...
첫 탭 「현장 목록」은 등록 현장별 내역서·매핑 현황, 산정 공기·판정(`site.last` 스냅샷), 서버 검토 건 상태·버전(`/api/reviews`)을 한 표로 보여준다. 현장을 전환하면 `stashWork()`가 현재 현장의 내역서·매핑·선후행·보고서 상태를 그 현장에 보관하고 `switchSite()`가 대상 현장의 것을 복원하므로 여러 검토 건을 오가며 처리할 수 있다(브라우저 저장에 포함).
