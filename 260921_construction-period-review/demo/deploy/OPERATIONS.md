...
### 5-1. 표준품셈 연간 개정 반영 절차 (매년 1회, 약 반나절)

표준품셈은 국토교통부·한국건설기술연구원이 매년 개정합니다. 개정본이 나오면 아래 순서로 반영합니다. 개발자 없이 담당자가 수행할 수 있게 설계했으며, 파일럿 납품 시 함께 1회 시연합니다.

1. 개정본 PDF를 `data/pumsem2026.pdf` 자리에 연도 이름으로 둔다(예: `pumsem2027.pdf`). 절 색인 파일 이름도 같이 맞춘다.
2. 정제 파이프라인을 실행한다. 원문 표에서 일당 시공량·품을 자동 추출해 `data/standards_official.js`와 추출 보고서(`pumsem_extract_report.json`)를 새로 만든다.
   ```bash
   python3 server/pumsem_extract.py
   ```
3. 추출 보고서의 "변경된 항목" 목록을 확인한다. 값이 바뀐 항목은 화면 「표준품셈」 탭에서 "자동 추출" 배지로 표시되므로, 자주 쓰는 공종부터 원문 페이지 링크로 대조해 검수한다. 검수한 항목은 `data/standards.js`(검수본)로 옮긴다.
4. 기존 검토 건은 저장된 버전의 값으로 유지된다(개정 전 기준으로 산정한 보고서가 바뀌지 않음). 새 검토 건부터 개정값이 적용된다.
5. `docker compose up -d --build`로 반영하고, 샘플 내역서 1건으로 매핑·작업일수가 정상인지 확인한다.

국토부 산정기준(가이드라인) 개정 시에는 `data/molit_basis.js`의 해당 값(준비기간 표, 기상조건 예시, 부록 값)을 원문대로 고치고 같은 방법으로 반영합니다.
