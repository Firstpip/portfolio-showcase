...
const STATUS_META = {
  ...
  contracted:   { label: '계약 완료',  emoji: '📝', color: '#0984e3' },
...
// ─── Button style helpers — 인라인 style 객체 중복 제거용 ───
const btn = {
  primary: (over={}) => ({ padding:'var(--space-2) var(--space-4)', borderRadius:'var(--radius-lg)',
    border:'none', background:'var(--accent)', color:'#fff', cursor:'pointer',
    fontSize:'var(--text-base)', fontWeight:600, ...over }),
  secondary: (over={}) => ({ padding:'var(--space-2) var(--space-3)', borderRadius:'var(--radius-lg)',
    border:'1px solid var(--border)', background:'transparent', color:'var(--text2)',
    cursor:'pointer', fontSize:'var(--text-base)', ...over }),
  ghost: (over={}) => ({ padding:'var(--space-2) var(--space-3)', borderRadius:'var(--radius-md)',
    border:'none', background:'transparent', color:'var(--text2)',
    cursor:'pointer', fontSize:'var(--text-base)', ...over }),
  danger: (over={}) => ({ padding:'var(--space-2) var(--space-3)', borderRadius:'var(--radius-lg)',
    border:'1px solid var(--surface-danger-mid)', background:'transparent', color:'var(--red)',
    cursor:'pointer', fontSize:'var(--text-base)', ...over }),
};
...
-- created_at을 TIMESTAMPTZ로 마이그레이션 (DATE 타입이면 시·분·초가 손실되어 같은 날짜 동률 발생)
-- 기존 DATE 값은 자정(00:00:00 UTC)으로 보존되며, 이후 신규 등록부터 시각까지 정확히 기록됨
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name='wishket_projects' AND column_name='created_at' AND data_type='date'
  ) THEN
    ALTER TABLE wishket_projects
...
-- 클라이언트가 loadData에서 RPC로 호출. 서버에서 단일 SQL UPDATE로 처리되어
-- history append race / 시계 불일치 / 중복 PATCH 문제 모두 해결.
...
function ConfirmModal({ state, onCancel }) {
  ...
  const accent = destructive ? 'var(--red)' : 'var(--accent)';
      ...
      <div ref={trapRef} role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{
          ...
          {confirmInput && (
            <div style={{ marginTop:12 }}>
              ...
              <input
                ...
                style={{ width:'100%', padding:'0.5rem 0.7rem', borderRadius:8, border:`1px solid ${inputMatch && inputVal ? 'var(--red)' : 'var(--border)'}`, background:'var(--surface2)', color:'var(--text)', fontSize:'0.85rem', outline:'none', fontFamily:'inherit' }}
              />
            </div>
          )}
        </div>
        <div style={{ display:'flex', gap:8, padding:'0.9rem 1.1rem 1rem', justifyContent:'flex-end' }}>
          <button onClick={onCancel}
            style={btn.secondary({ padding:'0.55rem 1.1rem' })}>{cancelLabel}</button>
          <button onClick={() => { onConfirm && onConfirm(); onCancel(); }} disabled={!inputMatch}
            style={btn.primary({
              padding:'0.55rem 1.2rem',
              background: inputMatch ? accent : 'var(--border)',
              color: inputMatch ? '#fff' : 'var(--text2)',
...
function LiveDot({ connected }) {
  return (
    <span title={connected ? 'Live · 실시간 동기화 중' : 'Offline · 연결 끊김'}
      style={{ width:7, height:7, borderRadius:'50%', display:'inline-block',
        background: connected ? 'var(--green)' : 'var(--red)',
        animation: connected ? 'pulse 2s infinite' : 'none', flexShrink:0 }} />
  );
}
...
function UserMenu({ session, currentMember, displayName, isAdmin, onTeamMgr, onShortcuts, onSignOut }) {
  ...
  const itemStyle = { display:'flex', alignItems:'center', gap:8, width:'100%', padding:'0.55rem 0.85rem',
    background:'transparent', border:'none', color:'var(--text)', cursor:'pointer', fontSize:'0.85rem',
    textAlign:'left', borderRadius:6 };
  const onItemHover = (e, on) => { e.currentTarget.style.background = on ? 'var(--surface2)' : 'transparent'; };
  const select = (fn) => () => { setOpen(false); fn && fn(); };

  return (
    <div ref={ref} style={{ position:'relative' }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ display:'flex', alignItems:'center', gap:6, padding:'0.3rem 0.5rem 0.3rem 0.35rem',
          borderRadius:8, border:'1px solid var(--border)', background: open?'var(--surface2)':'transparent',
          color:'var(--text)', cursor:'pointer', fontSize:'0.85rem' }}>
        {currentMember && <MemberAvatar member={currentMember} size={22} />}
        <span style={{ fontWeight:500 }}>{displayName}</span>
        <span style={{ color:'var(--text2)', fontSize:'0.7rem', marginLeft:2, transform: open?'rotate(180deg)':'none', transition:'transform 0.15s' }}>▾</span>
      </button>
      {open && (
        <div style={{ position:'absolute', top:'calc(100% + 6px)', right:0, minWidth:240,
          background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10,
          boxShadow:'0 12px 30px var(--shadow)', padding:6, zIndex:100,
          ...
          {isAdmin && (
            <>
              ...
              <button onClick={select(onTeamMgr)} style={itemStyle}
                onMouseEnter={e=>onItemHover(e,true)} onMouseLeave={e=>onItemHover(e,false)}>
                <span style={{ width:18 }}>👥</span> 팀원 관리
              </button>
            </>
          )}
          <div style={{ height:1, background:'var(--border)', margin:'2px 6px' }} />
          <button onClick={select(onShortcuts)} style={itemStyle}
            onMouseEnter={e=>onItemHover(e,true)} onMouseLeave={e=>onItemHover(e,false)}>
            <span style={{ width:18 }}>⌨︎</span> 단축키 도움말
            <kbd style={{ marginLeft:'auto', padding:'0.05rem 0.35rem', borderRadius:4, background:'var(--surface2)',
              border:'1px solid var(--border)', fontSize:'0.7rem', fontFamily:'monospace' }}>?</kbd>
          </button>
          <div style={{ height:1, background:'var(--border)', margin:'2px 6px' }} />
          <button onClick={select(onSignOut)} style={{ ...itemStyle, color:'var(--red)' }}
            onMouseEnter={e=>onItemHover(e,true)} onMouseLeave={e=>onItemHover(e,false)}>
            <span style={{ width:18 }}>↪</span> 로그아웃
          </button>
        </div>
      )}
    </div>
...
function Funnel({ data }) {
  ...
  const stages = [
    ...
    { key:'meeting_done', label:'논의', color:'var(--blue)' },
    { key:'won',          label:'수주', color:'var(--green)' },
  ];
...
function DBSetupBanner({ sql }) {
  ...
  return (
    <div style={{ padding:'1rem 1.25rem', borderRadius:10, background:'var(--surface-warning-soft)', border:'1px solid var(--surface-warning-mid)', marginBottom:'1rem' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
        <div style={{ fontSize:'0.85rem', fontWeight:600, color:'var(--yellow)' }}>⚙️ 팀원/담당자 기능 — DB 설정 필요</div>
        <button onClick={copy} style={{ padding:'0.25rem 0.6rem', borderRadius:6, border:'1px solid var(--surface-warning-mid)', background:copied?'var(--surface-warning-mid)':'transparent', color:'var(--yellow)', cursor:'pointer', fontSize:'0.8rem', fontWeight:600 }}>
          {copied ? '✓ 복사됨' : 'SQL 복사'}
        </button>
      </div>
  ...
  return (
    <div style={{ padding:'1rem', borderRadius:12, background:'var(--surface)', border:'1px solid var(--border)', marginBottom:16 }}>
      ...
      {duplicateWeeks.length > 0 && (
        <div style={{ marginTop:8, padding:'0.45rem 0.7rem', borderRadius:6, background:'var(--surface-danger-soft)', border:'1px solid var(--surface-danger-strong)', fontSize:'0.8rem', color:'var(--red)' }}>
      ...
      <div style={{ display:'flex', gap:8, marginTop:12, justifyContent:'flex-end', alignItems:'center' }}>
        {isEdit && onClearPlan && (
          <button onClick={() => onClearPlan(project.slug)} disabled={saving}
            style={btn.danger({ padding:'0.5rem 0.9rem', borderRadius:8, border:'1px solid var(--surface-danger-strong)', fontSize:'0.8rem', marginRight:'auto' })}>
            🗑️ 계획 전체 삭제
          </button>
        )}
        <button onClick={safeClose} style={btn.secondary({ padding:'0.5rem 1rem', borderRadius:8, fontSize:'0.85rem' })}>취소</button>
        <button onClick={handleSubmit} disabled={saving || !canSave}
          ...
          style={btn.primary({ padding:'0.5rem 1.2rem', borderRadius:8, fontSize:'0.85rem', background: canSave ? 'var(--accent)' : 'var(--border)', cursor: canSave ? 'pointer' : 'not-allowed' })}>
        ...
        return (
          <div key={week} style={{ borderRadius:7, border:`1px solid ${isToday ? 'var(--green)33' : isFocus ? 'var(--accent)30' : 'transparent'}`, background: rowBg, overflow:'hidden' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 8px', borderLeft:`3px solid ${rowBorderColor}` }}>
              ...
              {/* 진행률 숫자 */}
              <div style={{ flexShrink:0, fontSize:'0.8rem', fontWeight:600, color: allDone ? 'var(--green)' : blockedCount > 0 ? 'var(--red)' : 'var(--text2)', minWidth:36, textAlign:'right', opacity: empty ? 0.35 : 1 }}>
                {empty ? '—' : allDone ? '완료' : `${c.done}/${c.total}`}
              </div>
              {blockedCount > 0 && <span title={`${blockedCount}개 블로킹됨`} style={{ fontSize:'0.8rem', color:'var(--red)', flexShrink:0 }}>🔴</span>}
              {onFilterWeek && !empty && (
...
function MilestonePanel({ project, milestones, setupNeeded, teamMembers, saving, onUpdate, onDelete, onAdd, onReorder, onCreateFromTemplate, weeklyMilestones, onBulkCreateWeekly, onClearWeeklyPlan }) {
  ...
  if (setupNeeded) {
    return (
      <div>
        <div style={{ padding:'0.7rem 0.9rem', borderRadius:8, background:'var(--surface-warning-soft)', border:'1px solid var(--surface-warning-mid)', marginBottom:'0.75rem' }}>
    ...
    <div>
      ...
      <div style={{ padding:'0.75rem 0.9rem', borderRadius:10, background:'var(--surface2)', border:'1px solid var(--border)', marginBottom:'1rem' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
          ...
          <div style={{ fontSize:'0.9rem', fontWeight:700, color:hasBlocked?'var(--red)':'var(--green)' }}>{pct}%</div>
        </div>
        <div style={{ height:6, background:'var(--bg)', borderRadius:3, overflow:'hidden' }}>
          <div style={{ width:pct+'%', height:'100%', background:hasBlocked?'var(--red)':'var(--green)', transition:'width 0.3s' }} />
        </div>
        {hasBlocked && <div style={{ marginTop:6, fontSize:'0.8rem', color:'var(--red)' }}>🔴 블로킹된 작업이 있습니다</div>}
        {/* Weekly progress */}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
        {sorted.map((m, rowIdx) => {
          ...
          return (
            <div key={m.id} style={{ borderRadius:8, border:`1px solid ${m.blocked?'var(--surface-danger-strong)':'var(--border)'}`, background: m.blocked?'var(--surface-danger-soft)':'var(--surface2)', overflow:'hidden' }}>
              {/* Row */}
              <div style={{ display:'flex', alignItems:'center', gap:8, padding:'0.6rem 0.7rem' }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  {m.blocked && m.blocked_reason && !editing && (
                    <div style={{ fontSize:'0.8rem', color:'var(--red)', marginTop:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{m.blocked_reason}</div>
                  )}
                </div>
                {/* Right side */}
                ...
                <div style={{ display:'flex', flexShrink:0, borderLeft:'1px solid var(--border)', paddingLeft:4, marginLeft:2 }}>
                  ...
                  <button onClick={()=>onDelete(m)} style={iconBtn({ color:'var(--red)' })} title="삭제">✕</button>
                </div>
              </div>
                ...
                <div style={{ padding:'0.7rem 0.8rem', borderTop:'1px solid var(--border)', background:'var(--bg)', display:'flex', flexDirection:'column', gap:10 }}>
                  ...
                  <div>
                    <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:'0.8rem', color:'var(--text2)', cursor:'pointer' }}>
                      <input type="checkbox" checked={!!m.blocked} onChange={e=>onUpdate(m,{blocked:e.target.checked, blocked_reason:e.target.checked?m.blocked_reason:null})} style={{ accentColor:'var(--red)' }} />
                      🔴 블로킹 표시
                    </label>
                    {m.blocked && (
                      <input type="text" placeholder="블로킹 사유" defaultValue={m.blocked_reason||''}
                        ...
                        style={{ width:'100%', marginTop:6, padding:'0.4rem 0.6rem', borderRadius:6, border:'1px solid var(--surface-danger-strong)', background:'var(--surface2)', color:'var(--text)', fontSize:'0.8rem', outline:'none' }} />
                    )}
                  </div>
                  {/* Assignee */}
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
      ...
      <div>
        <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:'0.85rem', color:'var(--text2)', cursor:'pointer' }}>
          <input type="checkbox" checked={draft.blocked} onChange={e => set('blocked', e.target.checked)} style={{ accentColor:'var(--red)' }} />
          🔴 블로킹 표시
        </label>
        {draft.blocked && (
          <input type="text" placeholder="블로킹 사유" value={draft.blocked_reason} onChange={e => set('blocked_reason', e.target.value)}
            style={{ ...inputS, marginTop:6, border:'1px solid var(--surface-danger-strong)' }} />
        )}
      </div>
      ...
      <div style={{ borderTop:'1px solid var(--border)', paddingTop:16, display:'flex', gap:8, alignItems:'center' }}>
        {hasPending && (
          <button onClick={handleSave} disabled={saving || !draft.phase_label.trim()}
            style={btn.primary({ flex:1, padding:'0.65rem 1rem', borderRadius:8, fontSize:'0.9rem', fontWeight:700, opacity: saving ? 0.6 : 1 })}>
            {saving ? '저장 중...' : `저장 (${changedFields.length}개 수정)`}
          </button>
        )}
        {hasPending && (
          <button onClick={() => setDraft(initDraft(m))} disabled={saving}
            style={btn.secondary({ padding:'0.65rem 0.8rem', borderRadius:8, fontSize:'0.85rem' })}>
            되돌리기
          </button>
        )}
        <button onClick={() => onDelete(m)}
          style={btn.danger({ padding:'0.6rem 1rem', borderRadius:8, border:'1px solid var(--surface-danger-strong)', fontSize:'0.85rem', fontWeight:500, marginLeft: hasPending ? 0 : 'auto' })}>
          작업 삭제
        </button>
      </div>
    </div>
  ...
  if (setupNeeded) {
    return (
      <div style={{ maxWidth:900, margin:'0 auto', padding:'1.5rem 1rem' }}>
        ...
        <div style={{ padding:'0.9rem 1.1rem', borderRadius:10, background:'var(--surface-warning-soft)', border:'1px solid var(--surface-warning-mid)', marginBottom:'0.75rem' }}>
    ...
    <div style={{ maxWidth:1400, margin:'0 auto', padding:'1rem 1.25rem' }}>
      ...
      <div style={{ padding:'0.9rem 1.1rem', borderRadius:12, background:'var(--surface)', border:'1px solid var(--border)', marginBottom:14 }}>
        ...
        <div style={{ height:4, background:'var(--bg)', borderRadius:2, overflow:'hidden' }}>
          <div style={{ width:pct+'%', height:'100%', background:hasBlocked?'var(--red)':'var(--green)', opacity:0.6, transition:'width 0.3s' }} />
        </div>
        {hasBlocked && <div style={{ marginTop:8, fontSize:'0.8rem', color:'var(--red)' }}>🔴 블로킹된 작업이 있습니다</div>}
                  ...
                  {cards.length === 0 ? (
                    ...
                    return (
                      ...
                      <div
                        ...
                        style={{
                          borderRadius:8,
                          border: m.blocked?'1px solid var(--surface-danger-strong)':'1px solid var(--border)',
                          background: m.blocked?'var(--surface-danger-soft)':'var(--surface2)',
                        ...
                        <div style={{ display:'flex', alignItems:'flex-start', gap:8 }}>
                          ...
                          <button onClick={(e)=>{ e.stopPropagation(); onDelete(m); }}
                            ...
                            onMouseEnter={e=>{ e.stopPropagation(); e.currentTarget.style.opacity='1'; e.currentTarget.style.color='var(--red)'; }}
                        ...
                        {m.blocked && m.blocked_reason && (
                          <div style={{ fontSize:'0.8rem', color:'var(--red)', paddingLeft:23, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }} title={m.blocked_reason}>{m.blocked_reason}</div>
                        )}
...
// ─── MeetingPrepButton ───
// 데모 준비 파일(발표·대본·사전준비) 생성. Supabase 작업 큐에 job을 넣으면
// 맥북 데몬(claude -p)이 생성→Storage 업로드→서명 URL을 돌려준다. LLM 비용 0원.
function MeetingPrepButton({ project }) {
  const [status, setStatus] = useState('idle'); // idle | working | ready | error
  const [msg, setMsg] = useState('');
  const [readyUrl, setReadyUrl] = useState(null); // 완료된 job의 유효 서명 URL
  const cleanupRef = useRef(null);
  const touchedRef = useRef(false); // 사용자가 생성 버튼을 누르면 mount 복원이 덮어쓰지 않도록

  // 서명 URL(JWT) 안에 박힌 exp를 디코드해 만료 여부 판단 (TTL 변경과 무관하게 정확)
  const urlValid = (url) => {
    try {
      ...
      return exp * 1000 > Date.now() + 30000; // 30초 여유
    } catch { return false; }
  };
  ...
  const listen = (jobId) => {
    const finish = (url) => { teardown();
      if (url && urlValid(url)) { setReadyUrl(url); setStatus('ready'); setMsg(''); download(url); }
  ...
  const run = async () => {
    ...
    setStatus('working'); setMsg('요청 보내는 중…'); setReadyUrl(null);
    const { data: job, error } = await supabase
      .from('meeting_prep_jobs')
  ...
  // 모달 재진입 시: 이 slug의 최신 job을 조회해 상태 복원
  // - done + 서명 URL 유효 → 상시 다운로드 버튼
  // - pending/processing → 진행 중인 job 재구독 (완료 시 자동 다운로드)
  useEffect(() => {
    let alive = true;
    (async () => {
      const { data: rows } = await supabase
        .from('meeting_prep_jobs')
        .select('id,status,download_url,error')
        .eq('slug', project.slug)
        .order('created_at', { ascending: false })
        .limit(1);
      const job = rows && rows[0];
      if (!alive || touchedRef.current || !job) return;
    ...
    return () => { alive = false; if (cleanupRef.current) cleanupRef.current(); };
  }, [project.slug]);
  ...
  return (
    <div style={{ marginBottom:'0.75rem' }}>
      {status === 'ready' ? (
        <div style={{ display:'flex', gap:6 }}>
          <button onClick={(e) => { e.stopPropagation(); download(readyUrl); }}
            style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, flex:1, padding:'0.5rem 0.75rem', borderRadius:8, fontSize:'0.8rem', fontWeight:600, cursor:'pointer', background:'var(--green)', color:'#fff', border:'none' }}>
            ⬇️ 데모 준비 파일 다운로드
          </button>
          <button onClick={(e) => { e.stopPropagation(); run(); }} title="다시 생성"
            style={{ padding:'0.5rem 0.65rem', borderRadius:8, fontSize:'0.85rem', cursor:'pointer', background:'var(--surface2)', color:'var(--text2)', border:'1px solid var(--border)' }}>
            ↻
          </button>
        </div>
      ) : (
        <button onClick={(e) => { e.stopPropagation(); run(); }} disabled={status === 'working'}
          style={{ display:'flex', alignItems:'center', gap:6, width:'100%', padding:'0.5rem 0.75rem', borderRadius:8, fontSize:'0.8rem', fontWeight:500, cursor: status==='working'?'default':'pointer', background:'var(--surface2)', color:'var(--accent)', border:'1px solid var(--border)', opacity: status==='working'?0.7:1 }}>
          {status === 'working' ? '⏳' : '📑'} {status === 'working' ? (msg || '생성 중…') : '데모 준비 파일 생성'}
        </button>
      )}
      {status === 'error' && <div style={{ fontSize:'0.75rem', color:'var(--red, #e57373)', marginTop:4 }}>{msg}</div>}
    </div>
  );
}
      ...
      <div ref={trapRef} role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding:'1.25rem 1.5rem 0', flexShrink:0 }}>
          ...
          {HAS_MILESTONES.includes(project.current_status) && (!project.start_date || !project.deadline) && tab !== 'info' && (
            <div onClick={() => setTab('info')}
              style={{ marginBottom:8, padding:'0.5rem 0.75rem', borderRadius:8, background:'var(--surface-warning-soft)', border:'1px solid var(--surface-warning-strong)', color:'var(--yellow)', fontSize:'0.8rem', cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}
              title="클릭하면 정보 탭으로 이동">
              ⚠ {!project.start_date && !project.deadline ? '착수일·마감일 미설정' : !project.start_date ? '착수일 미설정' : '마감일 미설정'} — 정보 탭에서 입력
            </div>
          )}
                  ...
                  <div style={{ display:'flex', gap:10 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      {editDeadline && !POST_DEV.includes(project.current_status) && (() => {
                        ...
                        if (diff < 0) return <div style={{ marginTop:4, fontSize:'0.8rem', color:'var(--red)', opacity:0.9 }}>→ {-diff}일 초과</div>;
                        if (diff === 0) return <div style={{ marginTop:4, fontSize:'0.8rem', color:'var(--yellow)', opacity:0.9 }}>→ 오늘 마감</div>;
                  ...
                  <>
                    <div style={{ padding:'0.9rem 1rem', borderRadius:10, background:'var(--surface2)', border:'1px solid var(--border)' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                        ...
                        <div style={{ fontSize:'0.95rem', fontWeight:700, color:hasBlocked?'var(--red)':'var(--green)' }}>{pct}%</div>
                      </div>
                      <div style={{ height:6, background:'var(--bg)', borderRadius:3, overflow:'hidden' }}>
                        <div style={{ width:pct+'%', height:'100%', background:hasBlocked?'var(--red)':'var(--green)', transition:'width 0.3s' }} />
                      </div>
                      {current && (
                        <div style={{ marginTop:10, fontSize:'0.8rem', color:'var(--text2)' }}>
                          현재 작업: <b style={{ color: hasBlocked&&current.blocked?'var(--red)':MILESTONE_STAGES[current.status]?.color }}>
            ...
            <>
              ...
              {targets.length > 0 ? (
                <>
                  ...
                  {(() => {
                    // 'won'은 마일스톤 템플릿 자동 생성이 일어나므로 confirm.
                    const triggerSave = () => {
                      if (!newStatus) return;
                      if (newStatus==='won' && onRequestConfirm) {
                        const sm = STATUS_META[newStatus];
                        onRequestConfirm({
                          title: `${sm.emoji} ${sm.label}으로 전환`,
                          message: '수주 성공으로 전환하면 마일스톤 템플릿이 자동 생성됩니다.\n계속할까요?',
                          confirmLabel: `${sm.label} 전환`,
                          destructive: false,
                          onConfirm: () => onSave(project, newStatus, note),
                        });
                      } else {
                        onSave(project, newStatus, note);
                      }
                    };
                    return (
                      <>
                        ...
                        <button onClick={triggerSave} disabled={!newStatus||saving}
        ...
        {/* Footer — 어떤 탭/상태에서도 항상 접근 가능한 파괴적 액션 */}
        <div style={{ padding:'0.75rem 1.5rem', borderTop:'1px solid var(--border)', flexShrink:0 }}>
          ...
          }} disabled={saving} style={{ width:'100%', padding:'0.5rem', borderRadius:8, border:'1px solid var(--surface-danger-strong)', background:'transparent', color:'var(--red)', cursor:'pointer', fontSize:'0.8rem', fontWeight:500 }}>
            ❌ 미선정·프로젝트 삭제
          </button>
        </div>
      </div>
...
function EmptyState({ icon, title, hint, primary, secondary }) {
  return (
    <div style={{
      ...
      padding:'2.5rem 1.5rem', gap:'0.75rem', textAlign:'center',
    }}>
      <div aria-hidden="true" style={{ fontSize:'2.4rem', lineHeight:1, opacity:0.85 }}>{icon}</div>
      <div style={{ fontSize:'0.95rem', fontWeight:600, color:'var(--text)' }}>{title}</div>
      {hint && <div style={{ fontSize:'0.8rem', color:'var(--text2)', maxWidth:360, lineHeight:1.5 }}>{hint}</div>}
      {(primary || secondary) && (
        <div style={{ display:'flex', gap:8, marginTop:'0.4rem' }}>
          {primary && (
            <button onClick={primary.onClick}
              style={btn.primary({ padding:'0.45rem 0.95rem' })}>{primary.label}</button>
          )}
          {secondary && (
            <button onClick={secondary.onClick}
              style={btn.secondary({ padding:'0.45rem 0.95rem', fontWeight:500 })}>{secondary.label}</button>
          )}
        </div>
      )}
    </div>
...
function ProjectTable({ data, filter, search, dateRange, onRowClick, sortKey, sortOrder, onSort, onBatchDelete, onMemoSave, memberFilter, teamMembers, milestones, milestonesSetupNeeded, onQuickApplyTemplate, onOpenProject, onStartAutorun, onOpenRegenerate, demoSaving, onAddNew, onResetFilters }) {
  ...
  const filtered = useMemo(() => {
    const sortFn = (a,b) => {
      ...
      else cmp = sortOrder==='desc' ? vb.localeCompare(va) : va.localeCompare(vb);
      if (cmp !== 0) return cmp;
      // 동률 시 id desc로 tie-break (DB INSERT 순서 보존 — 같은 created_at 날짜 안에서 최신 등록이 위로)
                ...
                <tr key={r.slug} onClick={() => onRowClick(r)}
                  ...
                  <td style={{ padding:'0.7rem 0.75rem 0.7rem 1rem', whiteSpace:'nowrap' }}>
                    <span style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'0.3rem 0.7rem', borderRadius:6, fontSize:'0.8rem', fontWeight:500, background:meta.color+'30', color:meta.color, border:`1px solid ${meta.color}44` }}>
                      {meta.emoji} {meta.label}
                      {r._stale && <span title={`지원 후 ${daysBetween(r.created_at,new Date())}일 경과`} style={{ marginLeft:2, width:6, height:6, borderRadius:'50%', background:'var(--red)', animation:'pulse 2s infinite' }} />}
                    </span>
                  </td>
                    ...
                    {meetingStr ? (() => {
                      ...
                      return (
                        <span style={{ display:'flex', alignItems:'center', gap:4 }}>
                          {urgLabel ? (
                            <span style={{ padding:'0.1rem 0.35rem', borderRadius:4, fontSize:'0.8rem', fontWeight:700, background:urgency==='today'?'var(--surface-warning-mid)':'var(--surface-info-mid)', color:urgency==='today'?'var(--yellow)':'var(--blue)', animation:urgency==='today'?'pulse 2s infinite':'none' }}>{urgLabel}</span>
                          ) : (
                            <span style={{ padding:'0.1rem 0.35rem', borderRadius:4, fontSize:'0.8rem', fontWeight:600, background:isDone?'var(--surface-success-mid)':'var(--surface-warning-mid)', color:isDone?'var(--green)':'var(--yellow)' }}>{isDone?'완료':'예정'}</span>
                          )}
      ...
      <div style={{ padding:'0.7rem 1rem', borderTop:'1px solid var(--border)', fontSize:'0.8rem', color:'var(--text2)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, flexWrap:'wrap' }}>
        <span>
          {filtered.length === 0
            ? '0건'
            : totalPages > 1
              ? `${pageStart+1}–${Math.min(pageStart+PAGE_SIZE, filtered.length)} / 전체 ${filtered.length}건`
              : `${filtered.length}건 표시 중`}
        </span>
        {totalPages > 1 && (() => {
          // 페이지 번호 윈도우: 현재 페이지 ±2, 처음·끝 항상 표시
          const window = new Set([1, totalPages, safePage-2, safePage-1, safePage, safePage+1, safePage+2].filter(p => p >= 1 && p <= totalPages));
...
function App({ session }) {
  ...
  const handleBatchDelete = useCallback(async (slugs) => {
    ...
    for (const slug of targets) {
      try {
        ...
        failSlugs.push(slug);
        console.error(`[batchDelete] ${slug} 실패:`, err);
      }
    }
    setData(prev => prev.filter(d => !targets.includes(d.slug) || failSlugs.includes(d.slug)));
    setMilestones(prev => {
      const next = { ...prev };
      targets.forEach(s => { if (!failSlugs.includes(s)) delete next[s]; });
      return next;
    });
    setSaving(false);
  ...
  // 미팅 도달한 모든 프로젝트가 대상 (현재 상태 무관, history에 interview 흔적이 있으면 포함)
  const avgInterviewDays = useMemo(() => {
    if (!data) return null;
    const samples = [];
    data.forEach(d => {
      if (!d.created_at) return;
      const interviewDate = (d.history||[]).find(h => h.status === 'interview')?.date;
      if (!interviewDate) return;
      samples.push(daysBetween(d.created_at, interviewDate));
    });
    if (samples.length === 0) return null;
    ...
    <div>
      ...
      <div className="fade-in" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem', flexWrap:'wrap', gap:'0.75rem', position:'relative', zIndex:50 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <h1 style={{ fontSize:'1.45rem', fontWeight:700, letterSpacing:'-0.02em' }}>퍼스트핍 워크룸</h1>
      ...
      {(() => {
        ...
        return (
          <div className="fade-in delay-2" style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:'1rem' }}>
            ...
            {avgInterviewDays && (
              <div style={kpiStyle} title="지원일 → '미팅 예정' 상태로 전환된 날까지의 평균 일수.&#10;즉 클라이언트 회신·미팅 일정 확정 시점 기준 (lead 응답 속도).&#10;※ 사용자가 status 갱신을 늦게 하면 실제보다 길게 집계됩니다.">
                <span style={{ color:'var(--text2)' }}>지원 → 미팅 잡힘</span>
                <span style={numStyle('var(--blue)')}>평균 {avgInterviewDays.avg}일</span>
                <span style={{ color:'var(--text2)', fontSize:'0.8rem' }}>({avgInterviewDays.n}건)</span>
              </div>
            )}
          </div>
        );
    ...
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'var(--overlay)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1100 }}>
      <div ref={trapRef} role="dialog" aria-modal="true" onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding:'1.25rem 1.5rem', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <button onClick={onClose} style={btn.ghost({ padding:0, fontSize:'1.1rem' })}>✕</button>
        </div>
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg)', position:'relative' }}>
      <div style={{ width:'100%', maxWidth:360, padding:'0 1.5rem' }}>
        <div style={{ textAlign:'center', marginBottom:'2rem' }}>
          <div style={{ fontSize:'1.5rem', fontWeight:700, letterSpacing:'-0.02em', marginBottom:6 }}>퍼스트핍 워크룸</div>
          <div style={{ fontSize:'0.85rem', color:'var(--text2)' }}>내부 전용 도구</div>
        </div>
        ...
        <form onSubmit={handleLogin} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus
              placeholder="name@firstpip.co.kr"
          ...
          {error && (
            <div style={{ padding:'0.6rem 0.85rem', borderRadius:8, background:'var(--surface-danger-soft)', border:'1px solid var(--surface-danger-strong)', fontSize:'0.8rem', color:'var(--red)' }}>
              {error}
            </div>
          )}
          <button type="submit" disabled={loading || isLocked} style={btn.primary({
            marginTop:4, padding:'0.7rem', borderRadius:10, fontSize:'0.95rem', fontWeight:700,
            background: isLocked ? 'var(--surface2)' : 'var(--accent)',
            color: isLocked ? 'var(--text2)' : '#fff',
            cursor:(loading||isLocked)?'not-allowed':'pointer',
            opacity:(loading||isLocked)?0.7:1, transition:'background 0.15s, opacity 0.15s',
          })}>
