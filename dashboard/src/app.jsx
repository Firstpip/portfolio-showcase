                    ...
                    const triggerSave = () => {
                      ...
                      if (newStatus==='won' && onRequestConfirm) {
                        ...
                        onRequestConfirm({
                          ...
                          message: `'${sm.label}'(으)로 전환하면 마일스톤 템플릿이 자동 생성됩니다.\n계속할까요?`,
...
function ProjectTable({ data, filter, search, dateRange, onRowClick, sortKey, sortOrder, onSort, onBatchDelete, onMemoSave, memberFilter, teamMembers, milestones, milestonesSetupNeeded, onQuickApplyTemplate, onOpenProject, onStartAutorun, onOpenRegenerate, demoSaving, onAddNew, onResetFilters }) {
  ...
  const cols = [
    ...
    { key:'budget',     label:CONTRACT_NET_STATUSES.includes(filter)?'계약대금':'예산',  sortable:true,  width:'110px' },
...
function App({ session }) {
  ...
  const avgWonDays = useMemo(() => {
    ...
    const SECURED = ['won','in_progress','maintenance_free','maintenance_paid','delivered','settled'];
    const samples = [];
    data.forEach(d => {
          ...
          <div className="fade-in delay-2" style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:'1rem' }}>
            ...
            {wonBudgetTotal > 0 && (
              <div style={kpiStyle} title="위시켓 수수료(지원가 500만 이하 25% / 초과 20%) 차감 후 실지급 대금 합산 (부가세 제외)">
                <span style={{ color:'var(--text2)' }}>수주대금</span>
