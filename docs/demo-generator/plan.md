...
- **depends_on**: T8.3
- **requires_test**: yes
- **파일**: `worker/generate-demo/tokens-to-tailwind.ts` (결정론적 모듈), `worker/test-tokens-to-tailwind.ts` (4 케이스), `worker/generate-demo/generate-app.ts` (통합), `worker/prompts/generate-app-foundation.md` (tailwind 항목 제거)
- **구현 메모**:
  - **`tokensToTailwindConfig(tokens) → string`**: 순수 함수, JSON.stringify 로 안전 escape. primary/secondary/surface/text/radius/fontFamily 6 필드 → theme.extend.colors / borderRadius / fontFamily.sans 매핑. foreground 는 `#FFFFFF` 고정 (대부분 도메인 색상에서 contrast 안전).
  - **fontStack uniqueness**: 입력 fontFamily 가 이미 fallback (Pretendard/system-ui/sans-serif) 안에 있으면 중복 제거.
  - **generate-app.ts 통합**: Pass 1 응답에서 tailwind.config.cjs 항목이 있으면 무시 (LLM 비결정성 제거) + 모듈이 직접 작성. final files 목록에도 모듈 결과로 교체.
  - **foundation prompt 정리**: "tailwind.config.cjs 작성하지 마라 — T8.4 모듈이 처리" 명시 + 검증 체크리스트 갱신 + class 이름 가이드 (bg-primary / text-text / font-sans / rounded) 유지.
  - **결정론성 효과**: LLM 출력 토큰 절약 (~500 bytes) + 비결정성 제거 + 향후 spec 변동에도 안정.
- **자동 검증 결과 (2026-04-27, 4/4 통과 / 보정 1사이클)**:
  - 케이스 A (매핑): 토큰 6 → tailwind 6 필드 정확 매핑 (598 bytes 생성)
  - 케이스 B (escape): 따옴표/백슬래시 입력에서 JSON.stringify 정확 escape (`#"AB"CD` / `#22\\33` / `Helvetica "Neue"`)
  - 케이스 C (font fallback uniqueness): "Pretendard" 입력 시 fontStack 중복 0
  - 케이스 D (vite build): 생성된 tailwind.config.cjs 로 build 2246ms, dist css 에 토큰 4 색 모두 포함 (rgb 형식 `rgb(124 58 237 / ...)`)
  - 1차 D 실패: tailwind 3.x 가 hex 를 `rgb(R G B / opacity)` 공백 구분 형식으로 emit (CSS Color Module L4) — 매칭 패턴이 `#hex` 만 검사해서 false negative. 매칭 추가로 fix.
- **last_failure**: —
...
- **test_spec**:
  - [x] 5개 파일짜리 dist/ push 후 GitHub raw URL 5개 모두 200
  - [x] 재배포 시 변하지 않은 파일은 SHA byte-identical (T5.1 패턴)
...
- **review_checklist** (자체 평가, 사용자 승인 대기):
  ...
  - [x] **strict 후보 — 폴백 동작으로 통과 판정** (사용자 승인). spec 은 frontend=`next` 를 정확히 뽑았고 런타임이 `vite-react-ts` 하나뿐이라 폴백했으며 근거는 `demo_artifacts.stack_decision.fallback_reason` 에 기록된다. **완전 일치는 T8.10(next runtime) 의 몫** — §8 이 Phase 8 첫 cut 을 "vite-react-ts 1개 + standard" 로 한정했고, T8.9~T8.11 이 전부 `depends_on: T8.8` 이라 여기서 막으면 데드락이 된다
...
- **test_spec**:
  - [x] 파일 목록에 항목 9 `src/pages/Home.tsx` 추가 — placeholder 금지 + "Pass 2 는 flow page 만 덮어쓴다" 이유 + 내용 지시(소개 → tier1 flow 카드 그리드 → tier2/3 목록 → 시드 요약 숫자, 30 LOC 이상). 품질 체크에도 항목 추가. vue/next 프롬프트의 홈 항목 유지도 회귀 검사
  - [x] LLM E2E — 배포된 홈에서 placeholder 문구 0건, 본문 613자, 진입 요소 20개(링크 18 + 버튼 2), 제목 3개, 콘솔 에러 0
  - [x] 피해 데모 `260904_webinar-member-site` 재생성(307s) → 홈 **126자 "생성 중..." → 613자 정식 화면**으로 복구
- **last_failure**: —
...
- **test_spec**:
  ...
  - [x] 대시보드: 워커 오프라인이면 `💤 워커 꺼짐` 비활성 버튼 + title 에 사유(마지막 신호 N분 전, 호스트명). 사용자 승인 완료
- **last_failure**: —
...
- **완료 조치**: `worker/.env.local` 에 `GITHUB_TOKEN` 추가(gh CLI OAuth 토큰). 워커 재시작 후 로드·레포 push 권한(200/true) 확인. 장기적으로는 이 레포에만 contents read/write 를 준 fine-grained PAT 로 교체 권장 — gh 토큰은 재인증 시 바뀌고 권한 범위가 넓다.
...
- **depends_on**: T8.8
- **requires_test**: yes
- **파일**: `worker/generate-demo/mobile-frame.ts` (신규), `worker/prompts/modes/mobile-web.md` (신규), `worker/generate-demo/generate-app.ts`, `worker/generate-demo/orchestrator.ts`, `worker/test-mobile-frame.ts` (신규)
- **해야 할 일**: 모바일 앱 공고 처리 — 375px frame 안에 SPA를 시뮬레이션하는 wrapper layout. generate-app 프롬프트에 mobile-web 모드 분기 추가.
- **설계**: 프레임은 **코드로 결정론적 주입**(`mobile-frame.ts`), LLM 에는 "그 안에 들어갈 내용" 만 프롬프트 조각으로 맡긴다. Phase 8 내내 확인된 대로(T8.3b/T8.8b/T8.10b) 규칙을 부탁하면 확률적으로 빠지기 때문. body 자체를 폰 폭으로 만들어 스택 무관하게 동작하고(마운트 지점은 다르지만 body 는 항상 있다) toast 같은 포털도 프레임 안에 머문다. 모드 조각은 스택과 직교해 프롬프트를 모드×스택으로 복제하지 않고 뒤에 덧붙인다.
- **LLM E2E 실측**: probe(캐시 spec 의 demo_mode 만 mobile-web 으로, flow 4개) 201s → 배포본 헤드리스 측정에서 1440px 뷰포트 본문 폭 390px·가운데 정렬, **하단 고정 탭바 1개**, `<table>` 0개, 가로 스크롤 없음, 콘솔 에러 0. `demo_artifacts.demo_mode='mobile-web'`, `mobile_frame_applied=true` 기록. probe 는 DB·레포·로컬 정리 완료
- **운영 발견**: launchd 워커는 **코드 변경을 자동 반영하지 않는다**. 첫 E2E 가 T8.9 이전 코드로 돌아 프레임이 빠졌다. 코드 수정 후에는 `launchctl kickstart -k gui/$(id -u)/co.firstpip.demo-worker` 로 재시작해야 한다
- **test_spec** (plan 에 비어 있어 T8.9 착수 시 정의):
  - [x] `deriveDemoMode` 10케이스 — 대소문자·공백 허용, 모르는 값/숫자/null/배열 전부 standard 폴백
  - [x] 스택 3종 모두 정확한 전역 스타일시트에 주입, 기존 스타일은 앞에 보존, 스타일시트 부재 시 사유와 함께 실패 보고
  - [x] 마커 기반 idempotent — 3스택 모두 2회차 무변경
  - [x] 조각(885자)이 mobile-web 에만 덧붙고 standard 는 기존 프롬프트와 byte-identical
  - [x] 실제 빌드 4.6s, dist CSS 에 `max-width:390px` 반영, validate-dist 5항목 통과
  - [x] 1440px 뷰포트에서 본문 폭 390px·left=525px(가운데), 390px 뷰포트에서는 폭 전체 사용
...
- **마지막 업데이트**: 2026-09-09 (T8.13 DONE — Phase 8 전 task 완료)
- **완료된 task**: T0.1, T0.2, T0.3, T1.1, T1.2, T2.1, T2.2, T2.3, T2.4, T3.1, T3.2, T3.3, T3.4, T3.5, T4.1, T4.2, T4.3, T5.1, T5.2, T6.1, T6.2, T6.3, T7.1, T7.2, T8.0, T8.1, T8.2, T8.3, T8.4, T8.5, T8.6, T8.3b, T8.7, T8.8a, T8.8b, T8.8, T8.10, T8.10b, T8.12, T8.9, T8.11, T8.13
- **진행 중 task**: 없음
- **다음에 착수 가능**: 없음 — **Phase 8 전 task 완료**. 후속은 새 요구가 생길 때 신규 task 로
...
| 2026-09-09 | T8.13 완료 | React 데모의 홈 화면이 "생성 중..." placeholder 로 배포되던 문제 수정. 클라이언트가 링크를 열면 가장 먼저 보는 화면이라 치명적이었다. 원인: foundation 프롬프트가 라우팅 설명과 예시 코드에서는 `HomePage` 를 언급하지만 **"만들어야 할 파일" 목록에 `src/pages/Home.tsx` 항목이 없었다.** LLM 은 Home 을 flow placeholder 와 같은 취급으로 "생성 중..." 만 넣고, Pass 2 는 flow page 만 덮어쓰므로 그대로 배포된다. T8.10 에서 새로 쓴 vue/next 프롬프트는 Home 을 명시 항목으로 넣어 문제가 없었다 — React 프롬프트만의 누락이었다. 수정: 파일 목록에 항목 9 를 추가하고 placeholder 금지 + 덮어쓰지 않는 이유 + 구체 내용(소개 → tier1 카드 그리드 → tier2/3 목록 → 시드 요약 숫자)을 명시, 품질 체크 항목도 추가. 검증: 프롬프트 정적 8항목 + 피해 데모 `260904_webinar-member-site` 재생성(307s) 실측 — 홈이 **126자 placeholder → 613자 정식 화면**(진입 요소 20개·제목 3개·콘솔 에러 0)으로 복구. 교훈: 프롬프트의 산문 설명과 파일 목록이 어긋나면 LLM 은 목록을 따른다 — 요구사항은 목록에 항목으로 있어야 한다. |
